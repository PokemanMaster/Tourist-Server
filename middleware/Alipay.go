...
func pay(c *gin.Context) {
	// 该函数用于处理支付请求，首先生成一个唯一的交易号 tradeNo，
	var tradeNo = fmt.Sprintf("%d", xid.Next())
	//然后创建了一个 TradePagePay 对象 p
	var p = alipay.TradePagePay{}
	//设置了通知回调地址、返回地址、商品标题、商户订单号、订单金额和产品代码。
	...
	//接下来调用支付宝客户端的 TradePagePay() 方法生成支付链接 url，
	url, _ := client.TradePagePay(p)
	//并通过 c.Redirect() 将用户重定向到支付页面。
